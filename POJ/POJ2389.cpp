...
namespace FFT  // WARNING!!! do not reveal this namespace
{
    ...
    inline unsigned long int FastReverseBits(unsigned long int a, int NumBits) {
    ...
    void FFT(bool InverseTransform, const vector<complex<double> >& In, vector<complex<double> >& Out) {
        ...
        for (int i = 0; i < NumSamples; ++i) {
            Out[FastReverseBits((unsigned long int)i, NumBits)] = In[i];
        }
        // the FFT process
...
int main(void) {
    ...
    while(scanf("%s %s",sa,sb)==2) {
        ...
        FFT::convolution<unsigned char>(a, b, res);
        a.clear(), b.clear();
        res2.resize(digitN, 0);
        int d = min((int)res.size()-1, (int)maxN);
        for (int i=1; i<=d; ++i) {
            res2[i-1] = ((unsigned long int)floor(res[i]+eps));
        }
        res.clear();
